...
import com.coretex.shop.admin.model.digital.ProductFiles;
...
public class DigitalProductController {
	...
	public String getDigitalProduct(@RequestParam("id") String productId, Model model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		DigitalProductItem digitalProduct = digitalProductService.getByProduct(store, product);
	...
	public String saveFile(@ModelAttribute(value = "productFiles") @Valid final ProductFiles productFiles, final BindingResult bindingResult, final Model model, final HttpServletRequest request) throws Exception {
		...
		if (CollectionUtils.isNotEmpty(productFiles.getFile())) {
			LOGGER.info("Saving {} product files for merchant {}", productFiles.getFile().size(), store.getUuid());
	...
	public @ResponseBody
	ResponseEntity<String> removeFile(@RequestParam("fileId") String fileId, HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		try {
			...
			ProductItem product = digitalProduct.getProduct();
