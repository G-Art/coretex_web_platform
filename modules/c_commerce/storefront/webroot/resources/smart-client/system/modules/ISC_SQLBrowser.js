...
if (window.isc && window.isc.module_Core && !window.isc.module_SQLBrowser) {
    isc.module_SQLBrowser = 1;
    isc._moduleStart = isc._SQLBrowser_start = (isc.timestamp ? isc.timestamp() : new Date().getTime());
    if (isc._moduleEnd && (!isc.Log || (isc.Log && isc.Log.logIsDebugEnabled('loadTime')))) {
        isc._pTM = {
            message: 'SQLBrowser load/parse time: ' + (isc._moduleStart - isc._moduleEnd) + 'ms',
            category: 'loadTime'
        };
    ...
    isc.defineClass("SQLEditor", "VLayout");
    isc.A = isc.SQLEditor.getPrototype();
    isc.B = isc._allFuncs;
    isc.C = isc.B._maxIndex;
    isc.D = isc._funcClasses;
    isc.D[isc.C] = isc.A.Class;
    isc.A.sqlInputFormDefaults = {_constructor: "DynamicForm", height: 150, showResizeBar: true};
    isc.A.actionButtonsDefaults = {_constructor: "HLayout", layoutMargin: 5, membersMargin: 5, height: 20};
    ...
    isc.A.previewGridDefaults = {_constructor: "ListGrid", minFieldWidth: 100, autoFetchData: false};
    isc.B.push(isc.A.initWidget = function isc_SQLEditor_initWidget() {
            ...
            this.addAutoChild("sqlInputForm", {
                fields: [{
                    ...
                    keyPress: function (_2, _3, _4) {
                        if (_4 == 'Enter' && isc.EH.ctrlKeyDown()) {
                            if (isc.Browser.isSafari) _2.setValue(_2.getElementValue());
                            _1.execSQL();
        ...
        , isc.A.execSQL = function isc_SQLEditor_execSQL() {
            var _1 = this.sqlInputForm.getValue("sql");
            if (_1) {
        ...
        , isc.A.dsLoaded = function isc_SQLEditor_dsLoaded(_1) {
            var _2 = _1.ds;
            if (!this.previewGrid) this.addAutoChild("previewGrid", {dataSource: _2}); else this.previewGrid.setDataSource(_2);
    ...
    isc.A.previewGridStripDefaults = {
        ...
        members: ["autoChild:removeButton", "autoChild:addButton", "autoChild:exportButton", "autoChild:generateDSButton", "starSpacer", "autoChild:refreshButton", "autoChild:totalRowsIndicator"]
    };
    isc.B.push(isc.A.initWidget = function isc_SQLTableBrowser_initWidget() {
            this.Super("initWidget", arguments);
            var _1 = isc.DataSource.get("DataSourceStore");
            _1.performCustomOperation("dsFromTable", {
                schema: this.schema,
                dbName: this.dbName,
                tableName: this.config.name
            }, this.getID() + ".dsLoaded(data)")
        }
        , isc.A.dsLoaded = function isc_SQLTableBrowser_dsLoaded(_1) {
            this.dataSource = _1.ds;
            this.dataSourceXML = _1.dsXML;
            this.addAutoChild("previewGrid", {dataSource: this.dataSource});
            this.addAutoChild("previewGridStrip", {grid: this.previewGrid});
    ...
    isc.D = isc._funcClasses;
    isc.D[isc.C] = isc.A.Class;
    isc.A.sqlEditorDefaults = {_constructor: "SQLEditor"};
    isc.A.tablePaneDefaults = {_constructor: "SQLTableBrowser"};
    isc.B.push(isc.A.initWidget = function isc_DBPane_initWidget() {
        ...
        , isc.A.showTableBrowser = function isc_DBPane_showTableBrowser(_1) {
            var _2 = this.escapeForId(this.config.name + '_' + _1.name);
            this.showPane({ID: _2, title: _1.name, paneClass: "tablePane"}, _1)
        }
        , isc.A.escapeForId = function isc_DBPane_escapeForId(_1) {
            return isc.isA.String(_1) ? _1.replace(/(\/|\.)/g, '_') : _1
        }
        , isc.A.showPane = function isc_DBPane_showPane(_1, _2) {
            var _3 = this.getTab(_1.ID);
            if (_3) {
                this.selectTab(_3);
                return
            }
            ...
            isc.addProperties(_3, _1, {
                canClose: true,
                pane: this.createAutoChild(_1.paneClass, {config: _2, dbName: this.config.name})
            });
            this.addTab(_3);
            this.selectTab(_3);
            this.currentPane = _3.pane
        }
    );
    ...
    isc.B.push(isc.A.initWidget = function isc_DBSchemaTree_initWidget() {
        ...
        , isc.A.selectionChanged = function isc_DBSchemaTree_selectionChanged(_1, _2) {
            this.tableSelected(_1.name)
        }
        , isc.A.tableSelected = function isc_DBSchemaTree_tableSelected(_1) {
        }
        , isc.A.getExpansionComponent = function isc_DBSchemaTree_getExpansionComponent(_1) {
            var _2 = this.createAutoChild("detail", {
                sortField: "primaryKey",
                sortDirection: "descending",
                fields: [{
                    name: "name", title: "Column", formatCellValue: function (_3, _1) {
                        if (_1.primaryKey) return "<b>" + _3 + "</b>";
                        return _3
                    }
            ...
            isc.DMI.call("isc_builtin", "com.isomorphic.tools.BuiltinRPC", "getFieldsFromTable", _1.name, this.schema, this.serverType, this.db.name, function (_3, _4) {
                _2.setData(_4)
            });
            return _2
        }
        ...
        , isc.A.loadSchema = function isc_DBSchemaTree_loadSchema(_1) {
            this.db = _1;
            isc.showPrompt("Loading schema for database: " + _1.name);
            isc.DMI.call("isc_builtin", "com.isomorphic.tools.BuiltinRPC", "getTables", this.serverType, _1.name, true, true, this.catalog, this.schema, this.includeList, this.excludeList, this.getID() + ".loadSchemaReply(data)")
        }
        , isc.A.loadSchemaReply = function isc_DBSchemaTree_loadSchemaReply(_1) {
            isc.clearPrompt();
            for (var i = 0; i < _1.length; i++) {
                _1[i].name = _1[i].TABLE_NAME;
                _1[i].type = _1[i].TABLE_TYPE.toLowerCase()
            }
            this.setData(isc.ResultSet.create({dataSource: this.dataSource, allRows: _1}));
            this.sort("name");
            if (this.schemaLoaded) this.fireCallback("schemaLoaded")
        }
    );
    isc.B._maxIndex = isc.C + 7;
    isc.DBSchemaTree.registerStringMethods({schemaLoaded: ""});
    isc.DataSource.create({
        ID: "DataSourceStore",
        fields: [
            {
    ...
    isc.B.push(isc.A.initWidget = function isc_DBCompactList_initWidget() {
            this.fields = [{
                ...
                type: "select",
                width: 120,
                optionDataSource: "DBListDS",
                displayField: "name",
                valueField: "name",
                change: "if (this.form.databaseChanged) this.form.fireCallback('databaseChanged', 'dbName', [value])",
                valueMap: {}
            }];
            this.Super("initWidget", arguments)
        }
        ...
        , isc.A.setSelectedDB = function isc_DBCompactList_setSelectedDB(_1) {
            return this.setValue("dbName", _1)
        }
    );
    isc.B._maxIndex = isc.C + 3;
    isc.DBCompactList.registerStringMethods({databaseChanged: "dbName"});
    ...
    isc.B.push(isc.A.initWidget = function isc_DBList_initWidget() {
        ...
        , isc.A.dataArrived = function isc_DBList_dataArrived() {
            ...
            if (!this.initialCriteriaSet) {
                var _1 = {status: "OK"};
                this.setFilterEditorCriteria(_1);
                this.initialCriteriaSet = true;
                this.filterData(_1)
            }
            this.initialCriteriaSet = false
        }
        , isc.A.cellHoverHTML = function isc_DBList_cellHoverHTML(_1) {
            if (!this.hoverDV) this.hoverDV = isc.DetailViewer.create({
    ...
    isc.D = isc._funcClasses;
    isc.D[isc.C] = isc.A.Class;
    isc.B.push(isc.A.showWindow = function isc_c_SQLBrowser_showWindow(_1, _2) {
            isc.Window.create({
                title: "SQL Browser",
                width: "100%",
                height: "100%",
                ...
                items: [isc.SQLBrowser.create({autoDraw: false}, _2)]
            }, _1).show()
        }
    );
    ...
    isc.A.dbListDefaults = {
        ...
        autoFetchData: true,
        canHover: true,
        defaultFields: [{name: "name"}, {name: "status"}]
    };
    ...
    isc.A.mainLayoutDefaults = {_constructor: "HLayout", height: "*"};
    ...
    isc.A.dbPaneDefaults = {_constructor: "DBPane"};
    isc.B.push(isc.A.initWidget = function isc_SQLBrowser_initWidget() {
            this.Super("initWidget", arguments);
            this.dbList = this.createAutoChild("dbList", {selectionChanged: "if (state) this.creator.databaseChanged(record)"});
            ...
            this.leftSection = this.createAutoChild("leftSection", {
                sections: [{
                    ...
                    title: "Databases",
                    expanded: true,
                    controls: [this.dbListRefreshButton],
                    items: [this.dbList]
                }, {
                    name: "tables",
                    title: "Tables & Views",
                    expanded: true,
                    controls: [this.dbSchemaRefreshButton],
                    items: [this.dbSchemaTree]
                }]
            });
        ...
        , isc.A.showDBPane = function isc_SQLBrowser_showDBPane() {
            var _1 = this.db;
            this.showPane({ID: this.escapeForId("db_" + _1.name), title: _1.name, paneClass: "dbPane"}, _1)
        }
        , isc.A.databaseChanged = function isc_SQLBrowser_databaseChanged(_1) {
            if (_1.status == "OK") {
                this.db = _1;
                this.dbSchemaTree.loadSchema(_1);
                this.showDBPane()
            }
        }
        , isc.A.showTablePane = function isc_SQLBrowser_showTablePane(_1) {
            this.showDBPane();
            this.currentPane.showTableBrowser(_1)
        }
        , isc.A.escapeForId = function isc_SQLBrowser_escapeForId(_1) {
            return isc.isA.String(_1) ? _1.replace(/(\/|\.)/g, '_') : _1
        }
        , isc.A.showPane = function isc_SQLBrowser_showPane(_1, _2) {
            var _3 = this.rightPane.getTab(_1.ID);
            if (_3) {
                this.currentPane = _3.pane;
                this.rightPane.selectTab(_3);
                return
            }
            _3 = {};
            isc.addProperties(_3, _1, {canClose: true, pane: this.createAutoChild(_1.paneClass, {config: _2})});
            var _4 = this.rightPane.getTab(0);
            if (_4 && _4.name == "welcome") this.rightPane.removeTab(0);
            this.rightPane.addTab(_3);
            this.rightPane.selectTab(_3);
            this.currentPane = _3.pane
        }
    );
    isc.B._maxIndex = isc.C + 6;
    isc._moduleEnd = isc._SQLBrowser_end = (isc.timestamp ? isc.timestamp() : new Date().getTime());
    if (isc.Log && isc.Log.logIsInfoEnabled('loadTime')) isc.Log.logInfo('SQLBrowser module init time: ' + (isc._moduleEnd - isc._moduleStart) + 'ms', 'loadTime');
    delete isc.definingFramework;
} else {
    if (window.isc && isc.Log && isc.Log.logWarn) isc.Log.logWarn("Duplicate load of module 'SQLBrowser'.");
}
/*
 * Isomorphic SmartClient
 * Version v8.2p_2012-06-03 (2012-06-03)
