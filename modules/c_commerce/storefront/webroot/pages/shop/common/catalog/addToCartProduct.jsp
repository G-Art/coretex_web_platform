...
<script>
    ...
    $(function () {
        ...
        initProduct('<c:out value="${product.uuid}"/>', '#input-<c:out value="${product.uuid}" />');//pass form div
                    ...
                    <c:choose>
                            ...
                            <select id="${status.index}" name="${status.index}" class="attribute">
                                <c:forEach items="${option.values}" var="optionValue">
                                    <option value="${optionValue.uuid}" <c:if
                                            test="${optionValue.defaultAttribute==true}"> SELECTED</c:if>>${optionValue.name}
                                        <c:if test="${optionValue.price!=null}">&nbsp;<c:out
                                            value="${optionValue.price}"/></c:if></option>
                                </c:forEach>
                            </select>
                        </c:when>
                        <c:when test="${option.type=='radio'}">
                            <c:forEach items="${option.values}" var="optionValue">
                                ...
                                <input type="radio" class="attribute" id="${status.index}" name="${status.index}"
                                       value="<c:out value="${optionValue.uuid}"/>" <c:if
                                        test="${optionValue.defaultAttribute==true}"> checked="checked" </c:if> />
                                <c:out value="${optionValue.name}"/><c:if
                                    test="${optionValue.price!=null}">&nbsp; (<c:out
                                    value="${optionValue.price}"/>)</c:if><br/>
                            </c:forEach>
                        </c:when>
                        <c:when test="${option.type=='text'}">
                            <input type="text" class="attribute" id="${status.index}" name="${status.index}"
                                   class="input-large">
                        </c:when>
                        <c:when test="${option.type=='checkbox'}">
                            <c:forEach items="${option.values}" var="optionValue">
                                ...
                                <input type="checkbox" class="attribute" id="${status.index}" name="${status.index}"
                                       value="<c:out value="${optionValue.uuid}"/>"<c:if
                                        test="${optionValue.defaultAttribute==true}"> checked="checked" </c:if>  />
                                <c:out value="${optionValue.name}"/><c:if
                                    test="${optionValue.price!=null}">&nbsp; (<c:out
            ...
            <div class="store-btn form-inline">
                <c:if test="${product.quantityOrderMaximum==-1 || product.quantityOrderMaximum>1 && not product.productVirtual}">
                    <div class="form-group product-qty">
                        <input id="qty-productId-<c:out value="${product.uuid}" />"
                               class="input-mini form-control form-control-sm" placeholder="1" type="text">
                    </div>
                </c:if>
                <button class="btn addToCart addToCartButton btn-buy" type="button"
                        productId="<c:out value="${product.uuid}" />"><s:message code="button.label.addToCart"
