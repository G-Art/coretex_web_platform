    ...
    <div class="span12 common-row">
        ...
        <div class="span8 col-md-8 no-padding">
            <div class="checkout-box">
					<span class="box-title">
						<p class="p-title">
							...
							&nbsp;<a href="javascript:void(0)" onclick="editAddress('${customer.uuid}');"><s:message
                                code="label.generic.edit" text="Edit"/></a>
						</p>
					</span>

                <c:if test="${not empty customer}">

                    <form:form action="${editAddress}" method="GET" id="editBillingAddress_${customer.uuid}">
                        ...
                        <address>
                            ...
                            <c:choose><c:when test="${not empty customer.billing.stateProvince}"><c:out
                                    value="${customer.billing.stateProvince}"/></c:when><c:otherwise>
                                <script>$.ajax({
                                    url: "<c:url value="/shop/reference/zoneName"/>",
                                    type: "GET",
                                    data: "zoneCode=${customer.billing.zone}",
                                    success: function (data) {
                                        $('#billingZoneName').html(data)
                                    }
                                })</script>
                                <span id="billingZoneName"><c:out
                                        value="${customer.billing.zone}"/></span></c:otherwise></c:choose>,

                            <c:if test="${not empty customer.billing.country}">
                                <span id="customerCountryName"><script>$.ajax({
                                    url: "<c:url value="/shop/reference/countryName"/>",
                                    type: "GET",
                                    data: "countryCode=${customer.billing.country}",
                                    success: function (data) {
                                        $('#customerCountryName').html(data)
                                    }
                                })</script></span><br/>
                            </c:if>
            ...
            <div class="checkout-box">
									...
									&nbsp;
											...
											<c:if test="${not empty customer.delivery}">
                                                <a href="javascript:void(0)"
                                                   onclick="editShippingAddress('${customer.uuid}');"><s:message
                                                        code="label.generic.edit" text="Edit"/></a>
                                            </c:if>
                ...
                <c:if test="${not empty customer.delivery}">
                    <form:form action="${editAddress}" id="editShippingAddress_${customer.uuid}">
                        <input type="hidden" name="customerId" value="${customer.uuid}">
                        ...
                        <address>
                            ...
                            <c:choose><c:when test="${not empty customer.delivery.stateProvince}"><c:out
                                    value="${customer.delivery.stateProvince}"/></c:when><c:otherwise>
                                <script>$.ajax({
                                    url: "<c:url value="/shop/reference/zoneName"/>",
                                    type: "GET",
                                    data: "zoneCode=${customer.delivery.zone}",
                                    success: function (data) {
                                        $('#deliveryZoneName').html(data)
                                    }
                                })</script>
                                <span id="deliveryZoneName"><c:out
                                        value="${customer.delivery.zone}"/></span></c:otherwise></c:choose>,

                            <c:if test="${not empty customer.delivery.country}">
                                <span id="customerDeliveryName"><script>$.ajax({
                                    url: "<c:url value="/shop/reference/countryName"/>",
                                    type: "GET",
                                    data: "countryCode=${customer.delivery.country}",
                                    success: function (data) {
                                        $('#customerDeliveryName').html(data)
                                    }
                                })</script></span><br/>
                            </c:if>
                ...
                <c:if test="${empty customer.delivery}">
                    <form:form action="${editAddress}" id="addShippingAddress_${customer.uuid}">
                        <input type="hidden" name="customerId" value="${customer.uuid}">
                        <input type="hidden" name="billingAddress" value="false"/>
                        <a href="javascript:void(0)" onclick="addShippingAddress('${customer.uuid}');"><s:message
                                code="menu.addaddress" text="Add a new address"/></a> <br/>
                    </form:form>
                </c:if>
