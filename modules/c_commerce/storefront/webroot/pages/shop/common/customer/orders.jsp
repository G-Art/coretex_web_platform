                    ...
                    <c:when test="${not empty customerOrders.orders}">
                        <div id="shop">
                            ...
                            <table class="table table-striped">
                                ...
                                <c:forEach items="${customerOrders.orders}" var="order" varStatus="orderStatus">
                                    <tr><!-- item -->
                                        ...
                                        <td><fmt:formatDate type="both" value="${order.datePurchased}"
                                                            pattern="yyyy-MM-dd"/></td>
                                        <td><sm:monetary value="${order.total.value}"/>&nbsp;<small>
                                            (${fn:length(order.products)} <c:choose><c:when
                                                test="${fn:length(order.products)==1}"><s:message
                                                code="label.generic.item" text="item"/></c:when><c:otherwise><s:message
                                                code="label.generic.items" text="items"/></c:otherwise></c:choose>)
                                        </small>
                                        </td>
                            ...
                            <ul class="pagination">
                                ...
                                <c:forEach begin="1" end="${paginationData.totalPages}"
                                           varStatus="paginationDataStatus">
                                    <li class="${paginationData.currentPage eq (paginationDataStatus.index) ? 'active' : ''}">
                                        <a href="${ordersAction}?page=${paginationDataStatus.index}">${paginationDataStatus.index}</a>
                                    </li>
                                </c:forEach>
