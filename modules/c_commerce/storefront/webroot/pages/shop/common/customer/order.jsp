            ...
            <div class="row-fluid">
                        ...
                        <p class="nomargin">
                            <c:choose>
                                <c:when test="${order.orderStatus.value=='processed'}">
                                    <strong><s:message code="label.checkout.downloads.completed"
                                                       text="label.checkout.downloads.completed"/></strong><br/>
                                    <c:forEach items="${downloads}" var="download">
                                        <a href="<sm:orderProductDownload productDownload="${download}" orderId="${order.id}"/>"><c:out
                                                value="${download.fileName}"/></a>
                                    </c:forEach>
                                </c:when>
                                <c:otherwise>
                                    <s:message code="label.checkout.downloads.processing"
                                               text="*** An email with your file(s) download instructions will be sent once the payment for this order will be completed."/>
                                </c:otherwise>
                            </c:choose>
                        </p>
                    </c:if>
                ...
                <div class="col-md-4 col-sm-4 text-right pull-right">
                    <input type="button" onclick="print();"
                           value="<s:message code="label.generic.print" text="Print" />" name="Print"
                           class="btn btn-large">
                </div>
        ...
        <div id="printableOrder" class="row-fluid">
            ...
            <div class="row-fluid">
                ...
                <div class="col-md-12 col-sm-12 pull-left">
                    ...
                    <h2><s:message code="label.entity.order" text="Order"/>&nbsp;#&nbsp;<c:out
                            value="${order.id}"/><br/></h2>
                    <p class="lead">
            ...
            <div class="row-fluid">
                <div class="col-md-6 col-sm-6">
                    <c:if test="${not empty order.billing}">
                        <h5><strong><s:message code="label.customer.billingaddress" text="Billing address"/></strong>
                        </h5>
                        <p>
                ...
                <div class="col-md-6 col-sm-6">
                    <c:if test="${not empty order.delivery}">
                        <h5><strong><s:message code="label.customer.shippingaddress" text="Shipping address"/></strong>
                        </h5>
                        <p>
            ...
            <div id="cartContent">
                ...
                <div class="item head">
                    ...
                    <span class="productName"><s:message code="label.productedit.productname"
                                                         text="Product name"/></span>
                    ...
                    <div class="item">
                        <div class="cartImage">
                            <c:if test="${not empty product.image}">
                                <img width="60"
                                     src="<sm:shopProductImage imageName="${product.image}" sku="${product.sku}"/>"/>
                            </c:if>
                        </div>
                        ...
                        <span class="productName">
							<c:choose>
                                <c:when test="${product.product!=null}">
                                    <a href="<c:url value="/shop/product/" /><c:out value="${product.product.description.friendlyUrl}"/>.html"><c:out
							...
							<br/>
								<c:if test="${product.attributes !=null}">
									...
									<c:forEach items="${product.attributes}" var="attribute">
                                        <li><c:out value="${attribute.attributeName}"/>:&nbsp;<c:out
                                                value="${attribute.attributeValue}"/></li>
                                    </c:forEach>
									</ul>
                                </c:if>
							</span>
                ...
                <div class="total pull-right">
                    <c:forEach items="${order.totals}" var="orderTotal" varStatus="counter">
                        <small class="totalItem">
                            <c:if test="${orderTotal.code=='refund'}"><font color="red"></c:if>
                            <s:message code="${orderTotal.code}" text="${orderTotal.code}"/>:<c:if
                                test="${orderTotal.code=='refund'}"></font></c:if>
                            <span <c:if test="orderTotal.code=='total'">class="totalToPay"</c:if>><strong><c:if
                                    test="${orderTotal.code=='refund'}"><font color="red"></c:if>
                                <sm:monetary value="${orderTotal.value}"/>
                                <c:if test="${orderTotal.code=='refund'}"></font></c:if></strong></span>
                        </small>
                        <br/>
                    </c:forEach>
                </div>
