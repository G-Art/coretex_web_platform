...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        var searchTemplate = Hogan.compile([
            '<p class="suggestion-text"><span style="color: black; ">{{value}}</span></p>'
        ].join(''));
...
<script type="text/html" id="miniCartTemplate">
    <li>
            ...
            <tr id="{{productId}}" class="cart-product">
                ...
                <td>
                    <button productid="{{productId}}" class="close removeProductIcon"
                            onclick="removeItemFromMinicart('{{uuid}}')">x
                    </button>
                </td>
            </tr>
            {{/shoppingCartItems}}
        ...
        <div class="panel-body text-right">
            <a href="#" onclick="viewShoppingCartPage();" class="btn btn-group btn-success btn-minicart"><s:message
                    code="label.checkout" text="Checkout"/></a>
        </div>
        {{/code}}
        {{^code}}
        <h4 style="text-align: center;color:#666666"><s:message code="label.emptycart"
                                                                text="Your Shopping cart is empty"/></h4>
        {{/code}}
    </li>
</script>
...
<script type="text/html" id="miniCartSummaryTemplate">
    ...
    {{/code}}
    {{#code}}
    <span class="no-responsive uppercase"><s:message code="label.cart" text="Shopping cart"/></span> <font
            color="red"><strong>({{quantity}})</strong></font>
    {{/code}}
</script>
...
<script type="text/html" id="customerLoggedInAccountTemplate">
    <button type="button" class="btn dropdown-toggle" data-toggle="dropdown"><i class="fa fa-user no-desktop"></i> <span
            class="uppercase"><s:message code="label.generic.welcome" text="Welcome"/> <span class="no-responsive">{{firstName}}</span></span>
    </button>
    <ul class="dropdown-menu dropdown-menu-right dropdown-animation">
        <li>
            <a onClick="location.href='<c:url value="/shop/customer/dashboard.html"/>';" href="#"><i
                    class="fa fa-user"></i><s:message code="label.customer.myaccount" text="My account"/></a>
        </li>
        <li>
            <a onClick="location.href='<c:url value="/shop/customer/logout"/>';" href="#"><i
                    class="fa fa-power-off"></i><s:message code="button.label.logout" text="Logout"/></a>
        </li>
    </ul>
</script>
...
<script type="text/html" id="customerNotLoggedInAccountTemplate">
    <button type="button" class="btn dropdown-toggle" data-toggle="dropdown"><i class="fa fa-user no-desktop"></i><span
            class="uppercase"> <s:message code="button.label.signin" text="Signin"/></span></button>
    ...
    <ul id="signinPane" class="dropdown-menu dropdown-menu-right dropdown-animation">
        <li>
            ...
            <form class="login-form" id="login" method="post" accept-charset="UTF-8">
                ...
                <button id="login-button" type="submit" class="btn btn-group btn-dark btn-sm"><s:message
                        code="button.label.login" text="Login"/></button>
                <span></span>
                <br/>
                <a id="registerLink"
                   onClick="location.href='<c:url value="/shop/customer/registration.html"/>';" href=""
                   role="button" class="" data-toggle="modal"><s:message code="label.register.notyetregistered"
                    ...
                    <div class="header-top-dropdown">
                        ...
                        <c:if test="${requestScope.CONFIGS['displaySearchBox'] == true}">
                            <div id="searchFieldGroup" class="btn-group dropdown no-responsive">
                                <input id="searchField" class="typeahead form-control" name="q" type="text"/>"
                                autocomplete="off" spellcheck="false" dir="auto" value="<c:out value="${q}"/>">
                            </div>
                            <div class="btn-group dropdown">
                                <button type="button" class="btn dropdown-toggle no-responsive searchButton"
                                        data-toggle="dropdown"><i class="fa fa-search"></i><span
                                        class="uppercase"><s:message code="label.generic.search" text="Search"/></span>
                                </button>
                        ...
                        <c:if test="${requestScope.CONFIGS['allowPurchaseItems'] == true}">
                            <div id="miniCart" class="btn-group dropdown">
                                <button id="open-cart" type="button" class="btn dropdown-toggle" data-toggle="dropdown">
                                    <i class="fa fa-shopping-cart"></i> <span id="miniCartSummary"></span></button>
                                <!-- miniCartDetails id required to add cart content from html template -->
                                <ul id="miniCartDetails"
                                    class="dropdown-menu dropdown-menu-right dropdown-animation cart"></ul>
                            </div>
                        </c:if>
                    </div>
