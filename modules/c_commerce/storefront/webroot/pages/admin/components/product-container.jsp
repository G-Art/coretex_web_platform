        ...
        operationBindings: [
            {
                ...
                dataURL: "<c:url value="${containerRemoveUrl}" />&removeEntity=<c:out value="${removeEntity}" />"
            },
            ...
            data: [
                {
                    categoryId: "-1",
                    parentId: "0",
                    Name: "<s:message code="label.category.root" text="Root" />",
                    isFolder: true
                },
                <c:forEach items="${categories}" var="category" varStatus="status">
                {
                    categoryId: '<c:out value="${category.uuid}" />',
                    parentId: '<c:choose><c:when test="${category.parent!=null}"><c:out value="${category.parent.uuid}" /></c:when><c:otherwise>-1</c:otherwise></c:choose>',
    ...
    isc.HLayout.create({
        ...
        members: [
            isc.SectionStack.create({
                ...
                sections: [
                    {
                        title: "<s:message code="label.categories.title" text="Categories" />",
                        autoShow: true,
                        items: [categoryTree]
                    }
                ]
            ...
            isc.SectionStack.create({
                ...
                sections: [
                    {
                        title: "<s:message code="menu.catalogue-products" text="Products" />",
                        autoShow: true,
                        items: [itemList]
                    }
                ]
            ...
            isc.Img.create({
                src: "", width: 10, height: 32, layoutAlign: "center",
            ...
            isc.SectionStack.create({
                ...
                sections: [
                    {
                        title: "<s:message code="${componentTitleKey}" text="{componentTitleKey} UNDEFINED"/>",
                        autoShow: true,
                        items: [containerList]
                    }
                ]
