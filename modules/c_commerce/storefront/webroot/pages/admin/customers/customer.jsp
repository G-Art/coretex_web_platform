...
<div id="tabbable" class="tabbable">
    ...
    <h3>
        ...
        <c:choose>
            <c:when test="${customer.uuid!=null}">
    ...
    <c:if test="${customer.uuid!=null}">
        <div class="btn-group" style="z-index:400000;">
            <button class="btn btn-info dropdown-toggle" data-toggle="dropdown"><s:message
                    code="label.generic.moreoptions" text="More options"/> ... <span class="caret"></span></button>
            <ul class="dropdown-menu">
                <li><a id="${customer.uuid}" href="#resetPassword"><s:message code="button.label.resetpassword"
                                                                            text="Reset password"/></a></li>
                <li><a id="${customer.uuid}" href="#setCredentials"><s:message code="button.label.setcredentials"
                                                                             text="Set credentials"/></a></li>
            </ul>
        </div>
        <!-- /btn-group -->
        <br/>
    ...
    <form:form method="POST" modelAttribute="customer" action="${saveCustomer}">
        ...
        <form:hidden id="customerId" path="uuid"/>
        <form:hidden path="merchantStore.uuid"/>
        ...
        <address>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <form:input class="input-large highlight" id="bilstateOther" maxlength="100" name="bilstateOther"
                                path="billing.state"/>
            ...
            <div class="controls">
                ...
                <form:input id="billingPostalCode" cssClass="input-large highlight" maxlength="20"
        ...
        <div class="control-group">
            ...
            <div class="controls">
                ...
                <form:select class="input-large" items="${languages}" itemValue="uuid" itemLabel="code"
                             path="defaultLanguage.uuid"/>
        ...
        <address>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <form:input class="input-large" id="delstateOther" maxlength="100" name="delstateOther"
                                path="delivery.state"/>
        ...
        <c:if test="${fn:length(groups)>0}">
            <div class="control-group">
                ...
                <div class="controls">
                    <form:checkboxes cssClass="highlight" items="${groups}" itemValue="uuid" itemLabel="groupName"
                                     path="groups" delimiter="<br/>"/>
    ...
    <c:if test="${customerAttr.uuid!=null}">
        ...
        <c:if test="${options!=null && fn:length(options)>0}">
            ...
            <div id="attributesSuccess" class="alert alert-success" style="
            <c:choose>
            <c:when test="${success!=null}">display:block;</c:when>
                <c:otherwise>display:none;</c:otherwise></c:choose>"><s:message code="message.success"
                                                                                text="Request successfull"/></div>
            <div id="attributesError" class="alert alert-error" style="display:none;"><s:message code="message.error"
            ...
            <div id="attributesBox" class="box">
                ...
                <form id="attributes">
                    <input id="customer" type="hidden" value="<c:out value="${customerAttr.uuid}"/>" name="customer">
                            ...
                            <div class="controls">
                                <c:choose>
                                    <c:when test="${option.type=='Select'}">
                                        <select id="<c:out value="${option.uuid}"/>" name="<c:out value="${option.uuid}"/>">
                                            <c:forEach items="${option.availableValues}" var="optionValue">
                                                <option value="${optionValue.uuid}" <c:if
                                                        test="${option.defaultValue!=null && option.defaultValue.uuid==optionValue.uuid}"> SELECTED</c:if>>${optionValue.name}</option>
                                            </c:forEach>
                                        </select>
                                    </c:when>
                                    <c:when test="${option.type=='Radio'}">
                                        <c:forEach items="${option.availableValues}" var="optionValue">
                                            <input type="radio" id="<c:out value="${option.uuid}"/>"
                                                   name="<c:out value="${option.uuid}"/>"
                                                   value="<c:out value="${optionValue.uuid}"/>" <c:if
                                                    test="${option.defaultValue!=null && option.defaultValue.uuid==optionValue.uuid}"> checked="checked" </c:if> />
                                            <c:out value="${optionValue.name}"/>
                                        </c:forEach>
                                    </c:when>
                                    <c:when test="${option.type=='Text'}">
                                        <input class="textAttribute" type="text"
                                               id="<c:out value="${option.uuid}"/>-<c:out value="${option.availableValues[0].uuid}"/>"
                                               name="<c:out value="${option.uuid}"/>-<c:out value="${option.availableValues[0].uuid}"/>"
                                               class="input-large"
                                               value="<c:if test="${option.defaultValue!=null}">${option.defaultValue.name}</c:if>">
                                    </c:when>
                                    <c:when test="${option.type=='Checkbox'}">
                                        <c:forEach items="${option.availableValues}" var="optionValue">
                                            <input type="checkbox"
                                                   id="<c:out value="${option.uuid}"/>-<c:out value="${optionValue.uuid}"/>"
                                                   name="<c:out value="${option.uuid}"/>-<c:out value="${optionValue.uuid}"/>"
                                                    <c:if test="${option.defaultValue!=null && option.defaultValue.uuid==optionValue.uuid}"> checked="checked" </c:if>  />
                                            <c:out value="${optionValue.name}"/>
                                        </c:forEach>
                                    </c:when>
                    ...
                    <div class="form-actions">
                        <div class="pull-right">
                            <button type="submit" class="btn btn-success"><s:message code="button.label.save"
                                                                                     text="Save"/></button>
                        </div>
                    </div>
...
<div id="confirmModal" class="modal hide" style="z-index:600000" tabindex="-1" role="dialog"
  ...
  <div class="modal-body">
           <p id="modalMessage">
				<s:message code="label.customer.resetpasswor.confirm"
                           text="Are you sure you want to reset the customer password?"/>
           </p>
  </div>  
  <div class="modal-footer">
           ...
           <button class="btn cancel-modal" data-dismiss="modal" aria-hidden="true"><s:message
                   code="button.label.cancel" text="Cancel"/></button>
           <button class="btn btn-success close-modal" id="closeModal" data-dismiss="modal" aria-hidden="true"
                   style="display:none;"><s:message code="button.label.close" text="Close"/></button>
  </div>
  </span>
</div>


<div id="credentialsModal" class="modal hide" style="z-index:650000" tabindex="-1" role="dialog"
     aria-labelledby="myModalLabel" aria-hidden="true">
