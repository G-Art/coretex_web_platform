...
<%@ page session="false" %>
<%--@elvariable id="order" type="com.coretex.core.data.orders.OrderForm"--%>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $("#transactionsAction").click(function () {
            resetMessages();
            listTransactions('<c:out value="${order.uuid}"/>');
        });

        $("#sendInvoiceAction").click(function () {
            resetMessages();
            $('.sm').showLoading();
            sendInvoice('<c:out value="${order.uuid}"/>');
        });

        $("#updateStatusAction").click(function () {
            resetMessages();
            $('.sm').showLoading();
            updateStatus('<c:out value="${order.uuid}"/>');
        });

        $("#updateDownloadsAction").click(function () {
            resetMessages();
            $('.sm').showLoading();
            sendDownloadEmail('<c:out value="${order.uuid}"/>');
        });
        ...
        $("#captureAction").click(function () {
            ...
            $('.sm').showLoading();
            captureOrder('<c:out value="${order.uuid}"/>');
        });

        $(".close-modal").click(function () {
            location.href = "<c:url value="/admin/orders/editOrder.html" />?id=<c:out value="${order.uuid}"/>";
        });
        ...
        $("#refund").submit(function () {
            ...
            $.ajax({
                ...
                'success': function (result) {
                    ...
                    if (status == 0 || status == 9999) {
                        //window.location='<c:url value="/admin/orders/editOrder.html" />?id=<c:out value="${order.uuid}" />';
            ...
            <div class="sm-ui-component">
                ...
                <h3>
                    <div class="control-group">
                        <div class="controls">
                            <s:message code="label.order.id2" text="Order ID"/> <c:out value="${order.uuid}"/> - <span
                                class="lead"><s:message
                                code="label.order.${order.status}"
                                text="${order.status}"/></span>
                            <br>
                        </div>
                    </div>
                </h3>
                ...
                <div class="btn-group" style="z-index:400000;">
                    <button class="btn btn-info dropdown-toggle" data-toggle="dropdown"><s:message
                            code="label.generic.moreoptions" text="More options"/> ... <span class="caret"></span>
                    </button>
                    <ul class="dropdown-menu">
                        <li><a id="transactionsAction" href="#"><s:message code="label.order.transactions"
                                                                           text="Transactions list"/></a></li>
                        <li><a id="sendInvoiceAction" href="#"><s:message code="label.order.sendinvoice"
                                                                          text="Send email invoice"/></a></li>
                        <li><a id="updateStatusAction" href="#"><s:message code="label.order.updatestatus"
                                                                           text="Send order status email"/></a></li>
                        <li>
                            <c:if test="${downloads!=null}">
                                <a id="updateDownloadsAction" href="#"><s:message code="label.order.downloademail"
                                                                                  text="Send download email"/></a>
                            </c:if>
                        </li>

                        <!--<li><a href="<c:url value="/admin/orders/printInvoice.html?id=${order.uuid}" />"><s:message code="label.order.printinvoice" text="Print invoice"/></a></li>-->
                        <!-- available soon <li><a href="<c:url value="/admin/orders/printShippingLabel.html?id=${order.uuid}" />"><s:message code="label.order.packing" text="Print packing slip"/></a></li>-->
                        <li>
                            <c:if test="${customer!=null}">
                                <a href="<c:url value="/admin/customers/customer.html?id=${customer.uuid}"/>"><s:message
                                        code="label.order.editcustomer" text="Edit customer"/></a>
                            </c:if>
                        </li>
                    </ul>
                    &nbsp;
                    <c:if test="${order.total>0}">
                        <c:if test="${capturableTransaction!=null}">
                            <a id="captureAction" class="btn btn-primary btn-block" href="#"><s:message
                                    code="label.order.capture" text="Capture transaction"/></a>
                        </c:if>
                        <c:if test="${refundableTransaction!=null}">
                            <a id="refundAction" class="btn btn-danger btn-block" href="#"><s:message
                                    code="label.order.refund" text="Apply refund"/></a>
                        </c:if>
                    </c:if>
                ...
                <form:form method="POST" modelAttribute="order" action="${orderSave}">
                ...
                <div id="store.error" class="alert alert-error" style="display:none;"><s:message code="message.error"
                                                                                                 text="An error occured"/></div>
                ...
                <div class="span8">
                    ...
                    <div span="4">
                        ...
                        <div class="controls">
                            <form:input id="customerEmailAddress" cssClass="input-large highlight"
                                        path="customerEmailAddress"/>
                            <span class="help-inline"><form:errors path="customerEmailAddress"
                                                                   cssClass="error"/></span>
                        </div>

                        <address>
                            <label><s:message code="label.customer.shipping.firstName"
                                              text="Shipping first name"/></label>
                            <div class="controls">
                                <form:input cssClass="input-large" path="delivery.firstName"/>
                            </div>
                            <label><s:message code="label.customer.shipping.lastName"
                                              text="Shipping last name"/></label>
                            <div class="controls">
                                <form:input cssClass="input-large" path="delivery.lastName"/>
                            </div>
                            <label><s:message code="label.customer.shipping.streetaddress"
                                              text="Shipping address"/></label>
                            <div class="controls">
                                <form:input cssClass="input-large" path="delivery.address"/>
                            </div>
                            <label><s:message code="label.customer.shipping.city" text="Shipping city"/></label>
                            <div class="controls">
                                <form:input cssClass="input-large" path="delivery.city"/>
                            </div>
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    <form:select id="shippingZoneList" cssClass="shiiping-zone-list"
                                                 path="delivery.zone.code"/>
                                    <form:input class="input-large highlight" id="shippingZoneText" maxlength="100"
                                                name="shippingZoneText" path="delivery.state"/>
                                    <span class="help-inline"><form:errors path="delivery.zone.code"
                                                                           cssClass="error"/></span>
                                </div>
                            </div>
                            ...
                            <div class="controls">
                                <form:select cssClass="country-list" path="delivery.country.isoCode">
                                    <form:options items="${countries}" itemValue="isoCode" itemLabel="name"/>
                                </form:select>
                            </div>
                            <label><s:message code="label.customer.shipping.postalcode" text="Postal code"/></label>
                            <div class="controls">
                                <form:input cssClass="input-large" path="delivery.postalCode"/>
                            </div>
                        </address>
                ...
                <div class="span8">
                    ...
                    <div class="controls">
                        <form:input cssClass="input-large" path="datePurchased" class="small" type="text"
                                    data-date-format="<%=com.coretex.core.business.constants.Constants.DEFAULT_DATE_FORMAT%>"/>
                    ...
                    <label><s:message code="label.customer.order.agreement.title" text="Terms and conditions"/>:
                        <strong><c:out value="${order.customerAgreement}"/></strong></label>

                    <br/><br/>
                    <label><s:message code="label.order.paymentmode" text="Payment mode"/></label>
                    <div class="controls">
                        <strong><c:out value="${order.paymentType}"/> - <c:out
                                value="${order.paymentModuleCode}"/></strong>
                        <c:if test="${order.paymentType == 'creditcard' && order.creditCard != null}">
                            <br/><c:out value="${order.creditCard.cardType}"/> - <c:out
                                value="${order.creditCard.ccNumber}"/>
                        </c:if>
                        <br/><br/>
                    </div>

                    <c:if test="${order.shippingModuleCode!=null}">
                        <label><s:message code="label.order.shippingmethod" text="Shipping method"/></label>
                        <div class="controls">
                            <strong><c:out value="${order.shippingModuleCode}"/></strong><form:hidden
                                path="shippingModuleCode"/>
                        </div>
                    </c:if>
                ...
                <div class="span8" style="margin-top:20px;">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                        <c:forEach items="${order.orderProducts}" var="orderProduct" varStatus="counter">
                            <c:set var="total" value="${orderProduct.oneTimeCharge * orderProduct.productQuantity }"/>

                            <tr>
                                <td colspan="2">
                                    <c:out value="${orderProduct.productName}"/> - <a
                                        href="<c:url value="/admin/products/viewEditProduct.html?sku=${orderProduct.sku}"/>"><c:out
                                        value="${orderProduct.sku}"/></a>
                                    <c:if test="${fn:length(orderProduct.orderAttributes)>0}">
                                        <br/>
                                        <ul>
                                            <c:forEach items="${orderProduct.orderAttributes}" var="attribute">
                                                <li>${attribute.productAttributeName}
                                                    - ${attribute.productAttributeValueName}</li>
                                            </c:forEach>
                                        </ul>
                                    </c:if>
                                </td>
                                <td><c:out value="${orderProduct.productQuantity}"/></td>
                                <td><strong><sm:monetary value="${orderProduct.oneTimeCharge}"
                                                         currency="${order.currency}"/></strong></td>
                                <td><strong><sm:monetary value="${total}" currency="${order.currency}"/></strong>
                                </td>
                            </tr>

                        </c:forEach>

                        <c:forEach items="${order.orderTotal}" var="orderTotal" varStatus="counter">
                            <tr class="subt">
                                <td colspan="2">&nbsp;</td>
                                <td colspan="2">
                                    <c:if test="${orderTotal.orderTotalCode=='refund'}"><font color="red"></c:if>
                                    <s:message code="${orderTotal.orderTotalCode}" text="${orderTotal.orderTotalCode}"/>
                                    <c:if test="${orderTotal.orderTotalCode=='refund'}"></font></c:if></td>
                                <td><strong>
                                    <c:if test="${orderTotal.orderTotalCode=='refund'}"><font color="red"></c:if>
                                    <sm:monetary value="${orderTotal.value}" currency="${order.currency}"/>
                                    <c:if test="${orderTotal.orderTotalCode=='refund'}"></font></c:if></strong></td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    </table>
                ...
                <div class="span8">
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <form:select path="status">
                                <form:options items="${order.orderStatusList}"/>
                            </form:select>
                        </div>
                    </div>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <dl class="dl-horizontal">
                                <c:forEach items="${order.orderHistory}" var="orderHistory" varStatus="counter">
                                <c:if test="${orderHistory.comments!=null}">
                                <dd>
                                        <fmt:formatDate type="both" dateStyle="long" value="${orderHistory.dateAdded}"/>
                                    -
                                        <c:out value="${orderHistory.comments}"/>
                                    </c:if>
                                    </c:forEach>
                            </dl>
                        </div>
                    ...
                    <div class="form-actions">
                        <button type="submit" class="btn btn-medium btn-primary"><s:message code="button.label.save"
                                                                                            text="Save"/></button>
                    </div>
                </div>
                <br/>
...
<div id="transactionsModal" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true" style="width:900px;z-index:500000;">
    ...
    <div class="modal-footer">
        <button class="btn btn-primary" id="closeModal" data-dismiss="modal" aria-hidden="true"><s:message
                code="button.label.close" text="Close"/></button>
    </div>
</div>
...
<div id="refundModal" class="modal hide" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"
    ...
    <div class="modal-body">
        ...
        <div id="store.success" class="alert alert-success alert-success-modal" style="
        <c:choose>
        <c:when test="${success!=null}">display:block;</c:when>
        <c:otherwise>display:none;</c:otherwise></c:choose>"><s:message code="message.success"
                                                                        text="Request successfull"/></div>
        <div id="store.error" class="alert alert-error alert-error-modal" style="display:none;"><s:message
        ...
        <p>
            ...
            <span id="refundMessage" style="display:none;"><s:message code="" text=""/><span
                    id="refundAmount"></span></span>
        </p>
        <p>
        <form id="refund" class="form-inline">
            <label><s:message code="label.generic.amount" text="Amount"/></label>&nbsp;<input type="text" id="amount"
                                                                                              name="amount"
                                                                                              class="input-small"
                                                                                              placeholder="<s:message code="label.generic.amount" text="Amount" />">
            <input name="orderId" id="orderId" type="hidden" value="<c:out value="${order.uuid}"/>">
            <button id="refundButton" type="submit" class="btn btn-danger"><s:message code="label.order.refund"
                                                                                      text="Apply refund"/></button>
        </form>
    ...
    <div class="modal-footer">
        <button class="btn btn-primary close-modal" id="closeModal" data-dismiss="modal" aria-hidden="true"><s:message
                code="button.label.close" text="Close"/></button>
    </div>
</div