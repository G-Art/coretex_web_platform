...
public class ProductPriceUtils {
	 ...
	 * @return
	 */
	public BigDecimal getPrice(MerchantStoreItem store, ProductItem product, Locale locale) {
	 ...
	 *
	 * @param Product
	 * @param List<ProductAttributeItem>
	 * @return FinalPrice
	 */
	public FinalPrice getFinalProductPrice(ProductItem product, List<ProductAttributeItem> attributes) {
	 ...
	 * @return
	 */
	public FinalPrice getFinalPrice(ProductItem product) {
		...
		if (product.getAttributes() != null && product.getAttributes().size() > 0) {
			for (ProductAttributeItem attribute : product.getAttributes()) {
	...
	public String getStoreFormatedAmountWithCurrency(MerchantStoreItem store, BigDecimal amount) throws Exception {
		...
		if (store.getCurrencyFormatNational()!=null ? store.getCurrencyFormatNational():false) {
	...
	public String getFormatedAmountWithCurrency(Locale locale, CurrencyItem currency, BigDecimal amount) throws Exception {
		...
		Currency curr = Currency.getInstance(currency.getCode());
	 ...
	 * @throws Exception
	 */
	public String getAdminFormatedAmountWithCurrency(MerchantStoreItem store, BigDecimal amount) throws Exception {
		if (amount == null) {
			return "";
		}
		...
		Currency currency = Currency.getInstance(store.getCurrency().getCode());
		nf = NumberFormat.getInstance(Constants.DEFAULT_LOCALE);
		nf.setMaximumFractionDigits(Integer.parseInt(Character
				.toString(DECIMALCOUNT)));
		nf.setMinimumFractionDigits(Integer.parseInt(Character
	...
	/**
	 * Returns a formatted amount using Shopizer CurrencyItem
	 * requires internal java.util.CurrencyItem populated
	 *
	 * @param currency
	 * @param amount
	 * @return
	 * @throws Exception
	 */
	public String getFormatedAmountWithCurrency(CurrencyItem currency, BigDecimal amount) throws Exception {
		if (amount == null) {
			return "";
		}

		var instance = Currency.getInstance(currency.getCode());

		Validate.notNull(instance, "CurrencyItem must be populated with java.util.CurrencyItem");

		NumberFormat nf = null;


		Currency curr = instance;
		nf = NumberFormat.getInstance(Constants.DEFAULT_LOCALE);
		nf.setMaximumFractionDigits(Integer.parseInt(Character
				.toString(DECIMALCOUNT)));
		nf.setMinimumFractionDigits(Integer.parseInt(Character
	 ...
	 * @throws Exception
	 */
	public String getFormatedAmountWithCurrency(MerchantStoreItem store, BigDecimal amount, Locale locale)
			throws Exception {
	...
	public BigDecimal getAmount(String amount) {
		...
		if (!amount.contains(Character.toString(DECIMALPOINT))
			...
			if (matchPositiveInteger(amount)) {
				...
				throw new RuntimeException("Not a positive integer "
						+ amount);
			}
	...
	public BigDecimal getOrderProductTotalPrice(MerchantStoreItem store, OrderProductItem orderProduct) {
	...
	/**
	 * Determines if a ProductPriceItem has a discount
	 *
	 * @param productPrice
	 * @return
	 */
	public boolean hasDiscount(ProductPriceItem productPrice) {
	...
	private FinalPrice calculateFinalPrice(ProductItem product) {
	...
	private FinalPrice finalPrice(ProductPriceItem price) {
