...
public class ODSInvoiceModule implements InvoiceModule {
	...
	public ByteArrayOutputStream createInvoice(MerchantStoreItem store, OrderItem order, LanguageItem language) throws Exception {
		...
		File file = new File(order.getUuid() + "_working");
		...
		Map<String, ZoneItem> zns = zoneService.getZones(language);
		...
		if (store.getCountry() != null) {
			...
			CountryItem c = countries.get(store.getCountry().getIsoCode());
			if (c != null) {
		...
		//invoice number
		sheet.setValueAt(order.getUuid(), 3, 3);
		...
		if (order.getBilling().getZone() != null) {
			...
			ZoneItem billingZone = zns.get(order.getBilling().getZone().getCode());
			if (billingZone != null) {
		...
		if (order.getBilling().getCountry() != null) {
			...
			CountryItem c = countries.get(order.getBilling().getCountry().getIsoCode());
			if (c != null) {
		...
		//products
		Set<OrderProductItem> orderProducts = order.getOrderProducts();
		int productCell = PRODUCT_ROW_START;
		for (OrderProductItem orderProduct : orderProducts) {


			String orderProductName = ProductUtils.buildOrderProductDisplayName(orderProduct);

			sheet.setValueAt(orderProductName, 0, productCell);
		...
		//print totals
		productCell++;
		Set<OrderTotalItem> totals = order.getOrderTotal();
		for (OrderTotalItem orderTotal : totals) {
		...
		tempInvoiceName.append(order.getUuid()).append(TEMP_INVOICE_SUFFIX_NAME);
